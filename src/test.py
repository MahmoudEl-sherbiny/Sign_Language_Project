# ========================================
# REAL-TIME ASL RECOGNITION WITH CAMERA + MEDIAPIPE
# ========================================
...
MODEL_PATH = r"models\asl_gesture_model_final.h5"
...
def preprocess_frame(frame):
    """Resize + normalize the image like training"""
    ...
    while True:
        ...
        if result.multi_hand_landmarks:
            for hand_landmarks in result.multi_hand_landmarks:
                ...
                cv2.rectangle(frame, (x_min, y_min), (x_max, y_max), COLOR_CONFIDENT, 2)

        if roi is not None and roi.size > 0:
            processed_roi = preprocess_frame(roi)
            predictions = model.predict(processed_roi, verbose=0)
            predicted_class, confidence = get_smoothed_prediction(predictions, prediction_buffer)
            ...
            draw_text_with_background(frame, main_text, (50, 50), font_scale=2, thickness=3)
            draw_text_with_background(frame, conf_text, (50, 90), font_scale=1, thickness=2)

        else:
            draw_text_with_background(frame, "No hand detected", (50, 50), font_scale=1, thickness=2)
        ...
        draw_text_with_background(frame, f"FPS: {fps:.1f}", (10, 30), font_scale=0.7, thickness=2)
